...
if (nombrePaseadorEl) {
    ...
    function cargarPerrosAsignados(paseadorId, paseoActualId, activo) {
        ...
        get(perrosQuery).then(async snapshot => {
            const perros = snapshot.val() || {};
            const lista = Object.entries(perros);
            if (lista.length === 0) {
                dispEl.innerHTML = '<p style="font-size:13px;color:#666;text-align:center;">No tienes perros asignados.</p>';
            ...
            // Separar disponibles y en paseo
            const disponibles = [];
            const enPaseo = [];
            ...
            // Renderizar disponibles - AGRUPADOS POR CLIENTE
            if (disponibles.length === 0) {
                dispEl.innerHTML = '<p style="font-size:13px;color:#666;text-align:center;">Todos los perros están en paseo.</p>';
            } else {
                dispEl.innerHTML = "";
                // Agrupar por cliente
                const porCliente = {};
    ...
    // Tomar foto y subir a la NUBE (Firebase RTDB)
    ...
    // Tomar foto asociada a UN SOLO perro especifico (boton individual por perro)
    ...
    // Grabar video y subir a Firebase Storage
    window.grabarVideo = function () {
        const input = document.createElement("input");
        input.type = "file";
        input.accept = "video/*";
    ...
    // Boton unificado Foto/Video estilo WhatsApp
    // Tap corto -> foto | Mantener presionado (>350ms) -> video
    if (btnFoto) {
        const TEXTO_NORMAL = "📷 Toca: foto · Mantén presionado: video";
        const TEXTO_GRABANDO = "🎥 Suelta para grabar el video...";
        const UMBRAL_VIDEO_MS = 350;
        ...
        function iniciarPresion(e) {
            ...
            temporizadorPresion = setTimeout(function () {
                esVideo = true;
                btnFoto.classList.add("grabando");
                btnFoto.textContent = TEXTO_GRABANDO;
            }, UMBRAL_VIDEO_MS);
        }

        function terminarPresion(e) {
            if (btnFoto.disabled) return;
            if (temporizadorPresion) { clearTimeout(temporizadorPresion); temporizadorPresion = null; }
            btnFoto.classList.remove("grabando");
            btnFoto.textContent = TEXTO_NORMAL;
            if (esVideo) {
                grabarVideo();
            } else {
                tomarFoto();
        ...
        btnFoto.addEventListener("pointerdown", iniciarPresion);
        btnFoto.addEventListener("pointerup", terminarPresion);
        btnFoto.addEventListener("pointerleave", cancelarPresion);
        btnFoto.addEventListener("pointercancel", cancelarPresion);
        btnFoto.addEventListener("contextmenu", function (e) { e.preventDefault(); });
    }
        ...
        onValue(ref(db, "paseos"), snapshot => {
            ...
            if (activo) {
                ...
                if (mapaLeaflet && ubs.length > 0) {
                    ...
                    const m = L.marker([last.lat, last.lon]).addTo(mapaLeaflet);
                    m.bindPopup("📍 Paseador aqui").openPopup();
                    markers.push(m);
                }
            } else {
            ...
            activos.forEach(([paseoId, p]) => {
                ...
                if (mapaAdmin && p.ubicaciones) {
                    ...
                    if (ubs.length > 0) {
                        ...
                        const marker = L.marker([last.lat, last.lon]).addTo(mapaAdmin);
...
if (listaAgenda) {
    ...
    function mostrarAgenda(filtroId) {
        onValue(ref(db, "perros"), snapshot => {
            ...
            const filtrados = filtroId ? lista.filter(p => p.paseadorId === filtroId) : lista;
...
window.editarPerro = async function(perroId) {
    ...
    const nuevasObs = prompt("Observaciones:", perro.observaciones || "");
    const updates = {};
    if (nuevoNombre) updates.nombre = nuevoNombre.trim();
    ...
    if (nuevasObs !== null) updates.observaciones = nuevasObs.trim();
